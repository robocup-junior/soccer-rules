...
\added[id=TC]{Whenever a robot is removed from play, it must be turned off.}
...
This needs to be \replaced[id=TC]{confirmed by}{proved to} an OC member if a
...
\added[id=TC]{Robots must not produce visible light that may prevent the
...
\added[id=TC]{A team claiming that their robot is affected by the other team's
robot in any way must show the proof/evidence of the interference. Any
interference needs to be confirmbed by an OC member if a claim is placed by the
other team.}
...
All robots must have a stable \added[id=TC]{and easily noticeable} handle to
...
\replaced[id=TC]{All team members need to be within the minimum and maximum
