...
represent changes introduced this year.}}
...
\p After a visible and loud count\footnote{usually a count of
...
neutral spot \replaced[id=TC]{furthest from the ball, facing its own goal.}
...
\p After a robot has been fixed, it will be placed on the unoccupied neutral spot
\replaced[id=TC]{furthest from the ball, facing its own goal.}{nearest to
where it has been taken off, orientated towards the nearest wall.
...
\p \added[id=TC]{Whenever a robot is removed from play, it must be turned off.}
...
\p This needs to be \replaced[id=TC]{confirmed by}{proved to} an OC member if a
claim is placed by the other team.  A team claiming that their robot is
affected by colors has to show the proof/evidence of the interference.
...
\p Robots are not allowed to be colored orange, yellow, blue, or any other color
that can be confused with a landmark (see 4.3) in order to avoid interference.
Orange, yellow, blue \deleted[id=TC]{or any landmark-like} colored parts used
in the construction of the robot must either be occluded by other parts from
the perception by other robots or be taped/painted with a neutral color.
...
\p \added[id=TC]{Robots must not produce visible light that may prevent the
opposing team from playing when placed on a flat surface. Any part of a
robot that produces light that may interfere with the opposing robot's vision
system must be covered.}

\p \added[id=TC]{A team claiming that their robot is affected by the other team's
robot in any way must show the proof/evidence of the interference. Any
interference needs to be confirmed by an OC member if a claim is placed by the
other team.}
...
\p All robots must have a stable \added[id=TC]{and easily noticeable} handle to
hold and to lift them. The handle must be easily accessible \added[id=TC]{and
allow the robot to be picked up from 22 $\pm$ 2 cm of height.}
...
\p For RoboCupJunior 2019, there are two sub-leagues as follows
...
\p \replaced[id=TC]{All team members need to be within the minimum and maximum
age as specified in the RoboCupJunior General Rules which can be found at
...
\p Maximum team size is 4 members for RoboCupJunior 2019.
