...
at the inspection table does not count in favor of this time limit. {~~Teams that are late for the start of the game may be penalized one goal 
[strong]#per 30 seconds# at the referee’s discretion.~>Teams are considered late for the start of the game if they don't show up with at least one working robot. 
They may be penalized one goal [strong]#per 30 seconds# at the referee’s discretion. When the penalty has reached 10 -- 0, they automatically will lose the game.~~}
...
referee's discretion.~>Robot-to-robot or robot-ball-robot contact where the defender reaches the 'pushing'-line is considered pushing.
...
and their time penalty has passed, as described in ++}<<out-of-bounds>>{++, the referee will 
return the out of bounds robot(s) first before moving the ball. If the lack of progress
does not resolve after the return of the out of bounds robot(s) the previous defined rule
will kick in.++}
...
{++However, if the penalized robot scores an own goal, that goal will be counted against it.++}
...
neutral spot furthest from the ball, facing its own goal.~>After the penalty time has passed, the robot will be placed on the general area of its own corner.~~}
...
.  Place the robot on its back with the kicker facing up.
.  Place the ball inside the ball capturing zone.
.  Perform a vertical kick into the air.
.  The kick power test is passed if the ball height does not exceed 100 cm. 
