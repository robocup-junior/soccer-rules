...
RoboCupJunior Soccer consist of two sub-leagues: {~~Soccer Open~>Soccer Vision
(formerly Soccer Open)~~} and {~~Soccer Lightweight~>Soccer Infrared
...
* {~~Soccer Lightweight~>Soccer Infrared~~} is played using a special ball
that emits an IR signal and robots may weigh up to {~~1.4 kg~>1.5 kg~~}, and be
up to 22cm in size.

* {~~Soccer Open~>Soccer Vision~~} is played using a brightly colored orange
...
TIP: The Soccer league is planning to increase the team size to 5. As of 2026 the international 
...
[discrete]
=== Changes from the 2025 RoboCupJunior Soccer Rules
...
at the inspection table does not count in favor of this time limit. Teams 
are considered late for the start of the game if they don't show up with at least one working robot. 
They may be penalized one goal *per 30 seconds* at the referee’s discretion.
...
Robot-to-robot or robot-ball-robot contact where the defender reaches the 'pushing'-line is considered pushing.
...
After the penalty time has passed, robot will be placed on the general area of its own corner.
...
For {~~only the World Championships, starting in Brazil 2025,~>at least at the
...
{++Starting from the year 2026, Soccer Infrared is using a new
...
{~~Lightweight~>Infrared~~} and Soccer {~~Open~>Vision~~}

The matches in the Soccer {~~Open~>Vision~~}
sub-league are conducted using a (passive) orange golf ball, whereas the
matches in the Soccer {~~Lightweight~>Infrared~~} sub-league are played using
...
|===
|sub-league | Soccer {~~Open~>Vision~~} | Soccer {~~Lightweight~>Infrared~~}
|size ^[0]^  | 18.0 cm | 22.0 cm +
|height | 18.0 cm | 22.0 cm +
|weight | No limit | {~~1400 g~>1500g~~} +
|ball-capturing zone | 1.5 cm | {~~3.0 cm~>1.5 cm~~} +
|===
...
competition. During gameplay, a referee can {~~ask to see a sample kick~>test
