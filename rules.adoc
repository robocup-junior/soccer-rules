= RoboCupJunior Soccer Rules 2023 (Draft)
{docdate}
:toc: left
:sectanchors:
:sectlinks:
...
IMPORTANT: *IMPORTANT:* The field changes listed below in red are in a *DRAFT STATUS*.
Please expect further changes in the field and goal size & positions in the final version.

NOTE: *NOTE*: Please provide feedback regarding the draft changes on the forum:
https://junior.forum.robocup.org/t/enlarge-size-of-playing-area-soccer-2023-rules/2619/6

[[dimensions-of-the-field]]
=== Dimensions of the field

The playing-field is {~~132 cm by 193 cm~>162 cm by 223 cm.~~}. The field is
marked by a white line which is part of the playing-field. Around the
playing-field, beyond the white line, there is an outer area of {~~25 cm in width~>10 cm in width~~}.
...
or installed as white carpet and be somewhat robust to tearing or ripping.~~} {~~and have a width of 20 mm.~>Lines should have a width of 20mm (±10%).~~}
...
along the long edge of the field. They align with the sides of the goalie
...
Soccer Committee would like to encourage regional competitions to include
...
worthwhile to make it part of regional and super-regional competitions because a
number of regions are already running and report great success with them. Each
regional and super-regional competition will likely have its specific rules but
the Soccer Committee maintains a suggested ruleset to base Entry Leagues around.

- The Entry League should be roughly based on the RoboCupJunior Soccer rules
...
- The Entry League should ignore the Out of Bounds rule either by allowing play
  all the way to the walls or by placing robots that go out of bounds back in
  immediately.

- If may be considered to host an additional league for robots created exclusively
  from a specific set of permissible parts (e.g. only official Lego or Fischertechnik 
  kits, except for sensors necessary for robots to be able to find the ball (e.g. ball
  detector) and the orientation of the field (e.g. compass). For these robots a square
  permissible size of 22,4cm by 22,4cm by 22,4cm (all measurements +/- 1cm) have been
  used without a weight limit im some regional and super-regional tournaments.
