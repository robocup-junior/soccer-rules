= RoboCupJunior Soccer Rules {rules-year}
{docdate}
:toc: left
:sectanchors:
:sectlinks:
...
These are the Soccer rules for RoboCupJunior {rules-year}. They are released
...
RoboCupJunior Soccer consist of two sub-leagues: Soccer Vision
(formerly Soccer Open) and Soccer Infrared
(formerly Soccer Lightweight). These rules apply for both sub-leagues. There
are three main differences between the two leagues.

* Soccer Infrared is played using a special ball
that emits an IR signal and robots may weigh up to 1.5 kg, and be
up to 22cm in size.

* Soccer Vision is played using a brightly colored orange
golf ball, robots have no weight restriction and may be up to 18cm in size.
...
TIP: The Soccer league is planning to increase the team size to 5. {~~As of 2026-04-23
it is confirmed that a team size of 4 members will be the maximum amount of allowed team members 
within one team for the Incheon competition.~>As of 2026 the international 
competitions can guarentee a team size of 4, but no guarentees can be made for the team size of 5.~~}
We recommend team size 5 for SuperRegionals and
where organizers find it practical also for regional and local competitions.
Ask the organizers for each tournament you attend what team size they can
accomodate - tournament organizers are encouraged to adapt rules to fit with
their situation. Contact information for the world championships is: robocupjunior-soccer [ at \] robocup [ dot \] org

[[general-rules]]
include::general-rules/general-rules.adoc[]

[discrete]
=== Changes from the {previous-rules-year} RoboCupJunior Soccer Rules
...
are late for the start of the game~>Teams are considered late for the start of the game if they don't show up with at least one working robot.~~}
They may be penalized one goal *per 30 seconds* at the referee’s discretion. {++When the penalty has reached 10 -- 0, they automatically will lose the game.++}
...
(outside of the center circle). {++A neutral kick-off will occur when all robots from both teams are out 
of the field.++}
...
is moving, it is a good indication that the ball is trapped. {++If this happens during 
gameplay, the robot will be deemed damaged and will lose its inspection sticker until 
the robot complies with this rule.++}
...
If two robots from the same team are at least partially in {++their own++} penalty area,
...
referee's discretion.~>NOTE: THIS IMPLEMENTATION COULD CHANGE. PLEASE DO NOT DRAW THIS LINE FOR YOUR FIELD YET. Robot-to-robot or robot-ball-robot contact where the defender reaches the 'pushing'-line is considered pushing.
The 'pushing'-line will be an extra black line drawn within the penalty area. This line will be shown in an updated version 
of the field specification document.~~} In this case, the ball will be moved to the _furthest
unoccupied neutral spot_ immediately.

If a goal is scored as a result of a "pushing" situation, it will not be
granted.

If pushing and multiple defense happen at the same time the referees
will call and resolve the pushing situation first, then the multiple defense.
...
solve the lack of progress, the referee can call lack of progress again and
move the ball to a different neutral spot in the same way.

{++If the situation arises that one or multiple robots for either team are out of bounds
and their time penalty has passed, as described in the next section, the referee will 
return the out of bounds robot(s) first before moving the ball. If the lack of progress
does not resolve after the return of the out of bounds robot(s) then the referee wil move the ball.++}
...
robot is removed from the field for a {++minimum of++} one-minute. 
There is no time stoppage for the game itself. {~~The robot is
allowed to return if a kick-off occurs before the penalty has elapsed.~>The robot is
allowed to return whenever there is a game interruption after the initial one-minute penalty, i.e. kick-off, lack-of-progress, pushing etc.~~}

The one-minute penalty starts when the robot is removed from play.
Furthermore, any goal scored by the penalized robot will not be granted.
{++However, if the penalized robot scores an own goal, that goal will be awarded to the opposing team.++}
Out-of-bounds robots can be fixed if the team
needs to do so, as described in <<damaged-robots>>.

{~~After the penalty time has passed, robot will be placed on the unoccupied
neutral spot furthest from the ball, facing its own goal.~>After the penalty time has passed, 
the robot will be placed on the general area of one of its own corners, while not touching the white lines and facing its own goal.~~}

A referee can waive the penalty if the robot was accidentally pushed out of 
bounds {++or pushed into the ramp++} by an opposing robot. In such a case, 
the referee will call "pushed out" and may slightly push the robot back onto the field.
...
IMPORTANT: These interference rules mean that often when red lights (e.g. line
sensors) are detected by other teams' camera systems as the orange ball and
...
advised to make sure their robots do not have these bright red lights/parts
...
For at least at the
world championships (ask other competition organizers) use of a Communications
...
{~~Starting this year (2026)~>Starting from the year 2026~~}, Soccer Infrared is using a new
IR Ball. The key difference with this ball is the size change from 74mm to 42mm
diameter, which is the same size as the Vision League
golf ball.
This ball is Open Source, so anyone can produce one from
the files & instructions on the
GitHub Page here: https://github.com/robocup-junior/ir-golf-ball .
If you or someone you know can help by making or distributing balls for one
or more regions the Soccer Committee is interested in helping to make that
happen and listing you as a source for balls on the official website. Please
get in touch via Forum, Discord or Email
...
For RoboCupJunior, there are two sub-leagues: Soccer
Infrared and Soccer Vision

The matches in the Soccer Vision
sub-league are conducted using a (passive) orange golf ball, whereas the
matches in the Soccer Infrared sub-league are played using
the (also orange) IR ball.
...
|weight | No limit | 1500g +
|ball-capturing zone | 1.5 cm | 1.5 cm +
|===

TIP: [0] Robot must fit smoothly into a cylinder of this diameter. Being
3mm below the size limit and 50g below the weight limit will usually make
...
*FOR THE INFRARED LEAGUE ONLY:* Components designed to emit IR (e.g. 
ToF, LiDAR, IR distance sensors, IR LEDs/LASERs etc.) are not allowed and 
tournament organizers will require such devices to be removed or covered up.
...
competition. During gameplay, a referee can test
the kicker power on the
field before each half when a damaged robot is returned to the field or when
the game is about to be restarted after a goal. 
See <<kicker-power-measuring>> for more details.
...
international competition, it is run in many regions and at the SuperRegional
competitions (RoboCupJunior SuperRegional Americas, EMEA (Europe, Middle East,
Africa) and RoboCup Asia Pacific). 
To this end,
...
participate in.
{++Starting from the 2027 season we want to pilot a change in the kicker power measuring. We have noticed 
that different fields (or goal posts) give out different results. With some goal posts the posts absorb much 
of the bounce, with other goal posts the posts make the ball bounce back a lot. 

We want to try out a vertical kicker test that we feel will be much easier and much more consistent to see the 
actual kicker power.

The test is performed as follows:

1.  Place the robot on its back with the kicker facing up.
2.  Place the ball inside the ball capturing zone.
3.  Perform a vertical kick into the air.
4.  The kick power test is passed if the ball height does not exceed 100 cm. 
    You can measure this by performing the test near a wall with a measuring tape.++}
