...
{++The task in this technical challenge is to score a goal into an open goal
from one half of the field. In order for the goal to count, the ball can only
touch the other part of the field inside the penalty area and the goal itself.
Note that a golf ball (not necessarily orange) needs to be used.++}
