...
These are the Soccer rules for RoboCupJunior 2026. They are released
...
within one team for the Incheon competition. We recommend team size 5 for SuperRegionals and
...
{++Starting this year (2026), Soccer Infrared is using a new
...
.  Place robot inside a goal with the robot touching the back goal wall.
.  Perform a kick into the opposing goal
.  The kick power test is passed if after bouncing off the opposite goal the
ball does not hit the back wall of the goal the robot performed the kick power
test from.
